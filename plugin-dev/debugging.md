...
## ⑦ 插件被熔断（回调卡死）或自动禁用（导致崩溃）

框架对插件有两道**保护性隔离**。正常情况下你不会遇到；一旦遇到，说明插件里有需要修的东西。
...
```text
[watch] 插件 myplugin 的回调已阻塞 30 秒，判定为卡死 —— 已熔断，不再向它派发事件（重启服务可恢复）
```

并且管理员会收到一条「插件疑似卡死，已熔断」的通知。此后这个插件**收不到任何事件**。

**原因**：回调（`on_message` / 拦截器）里做了会长时间阻塞的事——最常见的是**同步等待外部 HTTP**，其次是死循环、等锁、等一个永远不会来的条件。

**处理方式**：
1. 找出回调里阻塞的那一步。给任何外部调用设**明确超时**（`http_get` / `http_request` 都带超时参数）；
2. 真正耗时的活儿**别放在回调里同步做**——先回一句「处理中」，把结果算好后用主动消息发出去；
3. 需要长驻任务（轮询、定时）的，不要自己起线程裸跑，用框架提供的接口（见 [生命周期](/plugin-dev/lifecycle)）；
4. 修好插件、重启服务即可恢复。

::: warning 为什么是"熔断"而不是"中断"
C++ 里**没有办法安全地杀掉一个卡住的线程**——强行取消会破坏栈展开和资源释放，后果比卡住更糟。

所以框架只能"识别并隔离"：卡住的那个线程会被放弃（占一个 worker），但这个插件从此不再收到事件，不会继续堆积；**其它插件完全不受影响**。

为了把被占住的线程也释放掉，熔断后框架会**自动重启一次服务**（数秒后，优雅停机、钩子照常执行）。
:::

### 情况二：插件导致服务崩溃被自动禁用

**症状**：服务重启后，插件市场里该插件变成**停用**，描述里多了「[自动禁用] 该插件曾导致服务崩溃……」；管理员收到同名通知。日志里能搜到：

```text
[crash] signal=11 (SIGSEGV) 调用栈 N 层；肇事插件=myplugin
[crash] 已写下肇事插件标记，下次启动会自动禁用它
```

**原因**：插件里的段错误（空指针、越界、悬垂指针、用错生命周期）把整个进程带崩了。**插件和框架在同一个进程里，这是唯一可能的后果。**

**处理方式**：按 `xbot.out` 里的调用栈用 `addr2line` 定位到自己的代码修掉，然后在插件市场重新启用。

::: tip 为什么框架要直接禁用它
不禁用的话，下次启动会在同一个地方**再崩一次**——表现就是「服务起来就死、反复重启」，把所有机器人和所有插件一起拖下水。

自动摘掉它，最坏结果只是「这个插件用不了」。
:::
...
| 保护 | 触发条件 | 结果 |
| --- | --- | --- |
| 异常护栏 | 插件回调抛出 C++ 异常 | 记录并忽略，**不影响其它插件** |
| 卡死看门狗 | 回调超过 **30 秒**未返回 | 熔断该插件 + 通知管理员 + 数秒后自动重启服务 |
| 崩溃归因 | 进程因插件段错误退出 | 定位肇事插件 → 下次启动自动禁用 + 通知 |
| 服务守护 | 进程异常退出 | 数秒内自动拉起 |

**但要说清楚边界**：这些保护让「一个坏插件」不至于拖垮整个系统，**不能让进程内崩溃变得无害**。写插件时仍然要假设自己随时可能把宿主带崩——这是共享进程模型的固有代价。
